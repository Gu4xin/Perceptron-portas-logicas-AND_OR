...
class Perceptron:
  # Declarando construtor da classe
    ...
    def train(self, inputs, outputs, learning_rate=0.1, epochs=100):
        ...
        # iniciando pesos iniciais de forma aleatória
        ...
        for i in range(epochs):
            for j in range(len(inputs)):
                ...
                sigmoid = 1 / (1 + np.exp( -(w1 * inputs[j][0] + bias)))

                w1 = w1 + learning_rate * (outputs[j][0] - sigmoid) * inputs[j][0]
                w2 = w2 + learning_rate * (outputs[j][0] - sigmoid) * inputs[j][0]
                bias = bias + (learning_rate * (outputs[j] [0] - sigmoid))

        return w1, w2, bias

    def predict(self, weights, x1, x2):
        # Podemos usar FUNÇÂO DEGRAU ou FUNÇÂO SIGMOID
        # SIGMOID = devolve um se o resultado for maior do que 0.5, senão 0
